...
\title{Supporting Information for "Structure and thermal evolution of exoplanetary cores"}
%
...
\includegraphics[width=\textwidth]{fig/r_ICend.pdf}
Radial fraction of the inner core ($r_{\rm IC}/r_{\rm OC}$) after 5~Gyr of evolution as a function of planetary mass, bulk iron content, mantle iron number (upper row: $\# Fe_{\rm M}=0$, lower row: $\# Fe_{\rm M}=0.1$), and core composition (left column: pure iron, right column: iron and $5\%$ light elements).

\noindent\textbf{Figure S2.}

$L_{ \rm \rho}$ (left) and $A_{\rm \rho}$ (right) for a range of planetary masses and iron contents. Values for Earth are $L_{ \rm \rho}=7683$~km and $A_{ \rm \rho}=0.484$ \cite{labrosse_thermal_2015}.

\includegraphics[width=\textwidth]{fig/Lrho_Arho.pdf}


\noindent\textbf{Figure S3.}

Inner core radius fractions at the beginning (end of accretion) and at the end ($5$~Gyr later) for planets with a mantle iron number $\#Fe_{\rm M}=0.2$. The core is made of pure iron (left panel) and iron with $5$~\% of light elements (right panel)

\includegraphics[width=\textwidth]{fig/R_IC_FeM20.pdf}
